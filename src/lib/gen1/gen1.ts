...
function parseGen1Box(data: Uint8Array, base: number, label?: string): Gen1BoxMon[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // DEBUG: Log move data for first few Pokemon
    if (i < 3 && label && label.includes("Current")) {
      console.log(`  [Move Debug] Slot ${i+1}:`)
      console.log(`    Raw moves (Gen1 indices): [${moves.join(', ')}]`)
      console.log(`    Move bytes: [0x${raw33[8].toString(16)}, 0x${raw33[9].toString(16)}, 0x${raw33[10].toString(16)}, 0x${raw33[11].toString(16)}]`)
    }
